...
var CourseStudiedApp = React.createClass({
    ...
    changeData: function (item) {
        ...
        this.refs.TabHeader.ChooseTab("coursestudied");
    },
    ...
    render: function () {
        var path = [{ title: 'Bài học hôm nay', status: 'active', link: '' }];;
        var tab=[];
        //Thêm một tab - id nên đăt theo đung luat de sau nay load duoc tu controller mac dinh
        tab.push({ Com: CourseStudiedList, id: 'coursestudied', suburl: 'coursestudied', title: 'Bài học hôm nay', parentTitle: 'Bài học hôm nay', path: path });
        var obj = window.getDefaultFromServer();
        var defaulttab = "coursestudied";
...
//        console.log('Default to:', defaulttab);
        return (
            <div>
                ...
                <TabHeader Tabs={tab} ref='TabHeader' defaulttab={defaulttab} basepath='/coursestudied/index' />
                </div>
        );
    }
});

ReactDOM.render(<CourseStudiedApp />, document.getElementById('container'));