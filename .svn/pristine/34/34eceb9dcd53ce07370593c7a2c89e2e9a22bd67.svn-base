...
*/
    onActive: function () {
        ...
        if (this.state.loaded === null || this.state.parentinfo !== objSubject) {
            this.setState({ parentinfo: objSubject });//Trường hợp có thay đổi thì nhận lại mã mới
            this.loadData();
        ...
        if (!!objSubject)
            this.props.parent.setParentTitle(this.props.parentTitle + '(' + objSubject.NAME + ')');
    },
...
*/
    onRowSelect: function (itemSelect) {
        ...
        if (itemSelect.length > 0) {
            this.props.parent.setKeyValue('courseCode', itemSelect[0]);
        } else {
            this.props.parent.setKeyValue('courseCode', null);
        }
    },
    /**
...
*/
    loadData: function (obj, selectobject) {
        ...
        var parentcode = this.props.parent.getKeyValue('subjectCode').CODE;
        $.ajax({
            url: "/Course/getAllSearch",
            dataType: 'json',
            data: {
                page: page,
                pageSize: pageSize,
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.ret > 0) {
                            ...
                            log.show("Bản ghi được xóa thành công", true);
                        } else {
                            if (data.ret === -2) {
                                log.show("Không thể xóa bản ghi này !", false);
                            }
                            if (data.ret === -3) {
                                log.show("Bad request !", false);
                            }
                        }
                    }.bind(this),
                    error: function(xhr, status, err) {
    ...
    render: function () {
        return (
                <div id={this.props.id+'_list'} className="tab-pane fade in active ">
                    ...
                    <CoursePopup setData={this.setData} 
