...
var ListQuestion = React.createClass({
    ...
    onActive: function () {
        ...
        this.loadData();
        document.getElementById(id + '_content_group').innerHTML =
            this.props.parent.getKeyValue('questiongroupcontent');
    },
    /**
    ...
    */
    ...
    onEditClick: function () {
        ...
        if (!this.state.rowSelect) {
            ...
            var ins;
            //sử dụng để xác định xem dạng dialog nào sẽ được hiển thị
            //1- GenPopUpQuestion
            //2-CoQuestiontion
            var Com;
            //sử dụng trong trường hợp thêm mới
                    ...
                    if (data.ret >= 0) {            
                        ...
                        switch (questiontype) {
                            case 'FG':
                                ...
                                break;
                            case 'FI':
                                this.replaceSelectedText("div", data.code,"FI");
                                break;
                            case 'FF':
