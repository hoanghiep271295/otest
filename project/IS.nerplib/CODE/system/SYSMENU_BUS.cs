...
select dbo.TCVN2Unicode(tenmh) tenmh1, dbo.TCVN2Unicode(hinhthuc) hinhthuc1 ,* from monhoc
        ...
        public List<SYSMENU_OBJ> getAdmin(string universitycode, string pris)
        {
            ...
            //where ISNULL(A.lock,0)=0 AND A.universitycode=@universitycode  and A.thetype='BACKEND' AND (B.prioritycode in ({0})  or ISNULL(a.parentcode,'')='' or A.prioritycode='ALL') order by a.theorder desc";
            //AND (B.prioritycode in ({0})  or ISNULL(a.parentcode,'')='' or A.prioritycode='ALL') 
            //AND A.universitycode=@universitycode
            string sql = @"with BA
AS
(
select  distinct A.* from sysmenu 
A LEFT join sysmenupriority B ON A.code=B.sysmenucode where ISNULL(A.lock,0)=0   and A.thetype='BACKEND' AND (B.prioritycode in ({0}))
)
...
@universitycode  and A.thetype='TOPMENU' AND (B.prioritycode in ({0})  or ISNULL(a.parentcode,'')='' or A.prioritycode='ALL') 
)
select distinct * from 
(
select * from BA
