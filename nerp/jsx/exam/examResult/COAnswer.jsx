...
componentDidMount: function () {document.getElementById(this.props.id + '_questiontypeCO').innerHTML =this.props.objQuestionGroup.CONTENT;},
render: function() {
var liQuestion = [];var id = this.props.id;var listQuestion = this.props.dataQuestion;var index = 1;
listQuestion.map((rowitem) => {
if (rowitem.QUESTIONGROUPCODE === id) {
liQuestion.push(<QuestionCO key={index} index={index} id={rowitem.CODE} objQuestion={rowitem} dataAnswer={this.props.dataAnswer} listStudentAnswer={this.props.listAnswerStudent} questiontype={this.props.questiontype} />); index++;}});
return (
<div><div className="container questiongroup"><b>{this.props.number}.</b></div>
<div><div><fieldset className="fieldsetGroup lineHeight"><span id={this.props.id + '_questiontypeCO' }></span><ul>{liQuestion}</ul></fieldset>
</div></div></div>);}});
var QuestionCO = React.createClass({
componentDidMount: function () {
document.getElementById(this.props.id + '_questionCO').innerHTML = this.props.objQuestion.CONTENT;
},
    ...
    CheckItemInArray(questioncode, arr) {
        var count = 0;
        for (var i = 0; i < arr.length; i++) {
            if (arr[i].QUESTIONCODE === questioncode) {return true;} count++;
    ...
    {   var listanswer = [];
            ...
            if (rowitem.QUESTIONCODE === idQuestion) {
                ...
                switch (this.CheckItem(rowitem, listStudentAnswer, rowitem.QUESTIONCODE))
                {
                    case 1:///mark is glyphicon-ok and radio button will be checked
                               listanswer.push(
                        <li key={indent} className='col-lg-3' id={rowitem.QUESTIONCODE}>
                    ...
                    case 2://mark is glyphicon-ok and  radio button will not be checked
                                listanswer.push(
                        <li key={indent} className='col-lg-3' id={rowitem.QUESTIONCODE} disabled>
        ...
        return (
            <div>
    <div className="container"><b className="col-sm-1">{this.props.index}.</b>
    <div className="col-sm-10 fitContent"><span id={this.props.id + '_questionCO'}></span></div></div>
    <ul className="list-unstyled container none-style list-inline"> {listanswer}</ul>
        </div>
        );}});