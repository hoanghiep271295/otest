var QuestionGroupMC = React.createClass({
    componentDidMount: function () {
        document.getElementById(this.props.id + '_questiontypeMC').innerHTML =this.props.objQuestionGroup.CONTENT;
    },
    render: function() {
var liQuestion = [];
...
var index = 0;
listQuestion.map((rowitem) => { if (rowitem.QUESTIONGROUPCODE === id) {index++;
liQuestion.push(<QuestionMC key={index}index={index}id={rowitem.CODE}objQuestion={rowitem}dataAnswer={this.props.dataAnswer}listStudentAnswer={this.props.listAnswerStudent}questiontype={this.props.questiontype} />);}});
return (
<div><div className="container questiongroup"><b>{this.props.number}.</b></div><div>
<div><fieldset className="fieldsetGroup lineHeight"><div className="text-center"><span id={this.props.id + '_questiontypeMC' }></span></div><ul>{liQuestion}</ul></fieldset></div>
</div></div>);}});
var QuestionMC = React.createClass({
    componentDidMount: function () {
        document.getElementById(this.props.id + '_questionMC').innerHTML = this.props.objQuestion.CONTENT;
    },
        ...
        listAnswer.map( (rowitem)=> {
               if (rowitem.QUESTIONCODE === idQuestion) {
                    ...
                    case 3://mark is glyphicon-remove and  radio button will be checked
                        listanswer.push(<li key={indent} className='flex-item none-style' id={rowitem.QUESTIONCODE}>
                          <input type="radio" id={rowitem.CODE} name={rowitem.QUESTIONCODE} disabled checked /> &nbsp; &nbsp; &nbsp;
                        <label id={indent} htmlFor={rowitem.CODE}>
                            <span key={'error_'+ indent} dangerouslySetInnerHTML={{ __html: answer }}></span>
