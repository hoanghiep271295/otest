...
*/
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.ret >= 0) {
                    ...
                    if (!!$('#subjectcontent_list_tree').jstree(true)) {
                        var jsonNodes = $('#subjectcontent_list_tree').jstree(true).get_json('#', { flat: true });
                        this.setState({currentNode: jsonNodes[0]});
                    ...
                    var that = this;
                    $("#subjectcontent_list_tree")
                        .on('loaded.jstree',
                            function () {
                                closed.forEach(function (item) {
                                    $("#subjectcontent_list_tree").jstree("close_node", item.id);

                                });
                                // vị trí scroll div tree trước khi load
                                var scrollTop = that.getKeyValue('ScrollTop');
                                // sau khi load xong thì đưa scroll trở lại đúng vị trí ban đầu
                                $("div#subjectcontent_list_tree").scrollTop(scrollTop);
                                // set node duoc chon tren cay
                                if (!!that.state.currentnode) {
                                    $("#subjectcontent_list_tree").jstree("open_node", that.state.currentnode[0].id);
                                    $('#subjectcontent_list_tree')
                                       .jstree(true)
                                    ...
                                    var firstnode = $('#subjectcontent_list_tree').jstree(true).get_json('#', { flat: true })[0];
                                    $('#subjectcontent_list_tree')
                                        .jstree(true)
                                        .select_node(firstnode);
                                }
                            });
                    this.setKeyValue('comboSubject', subjectCode);
    ...
    */
    TreeSelect: function (obj) {
        this.setKeyValue('ScrollTop', $("div#subjectcontent_list_tree").scrollTop());
    ...
    */
    ReloadList: function(subjectcode, obj) {
        ...
        this.loadTree(temp, obj);
       // setTimeout(function() {
       //     $('#subjectcontent_list_tree').jstree(true).select_node(this.state.currentnode[0]);
    ...
    */
            ...
            success: function (data) {
                ...
                if (data.ret >= 0) {
                   ...
                   // if (data.contenttype >= 0) {
                        obj.EXAMTIMECODE = data.subcontenttest.EXAMTIMECODE;
                   // }
    ...
    onPreviewClick: function () {
        if (this.state.rowSelect === null)
            ...
            window.open('/SubjectContent/Preview/' + currentcode, '_blank');
        }
    ...
    <div className="box">
        <div className="row">
            ...
            <div className="col-xs-3" style={{borderRight: '1px solid gray', minHeight:550}}>
                ...
                <a href="#" onClick={this.TreeSelect}>Danh mục nội dung bài học</a>
