var StudentList = React.createClass( {
    getInitialState: function() {
        return {
            ...
            columnDefs: [
                ...
                { headerName: "Ngày sinh", field: "BIRTHDAY", width: 350, editable: false, type: 'date' },
                { headerName: "Giới tính", field: "SEX", width: 350, editable: false },
    ...
    componentDidMount: function () {
        ...
        this.setState({ priority: this.refs[this.props.id + '_Priority'] });
    },
    ...
    */
    registerButton: function () {
        ...
        obj = [{ title: 'Phân quyền', callback: this.onKeyClick, icon: 'fa fa-key', ref: 'cmdKey' }
...
*/
    onActive: function (parent) {
        ...
        var objClass = this.props.parent.getKeyValue('class');
        // nếu chưa chọn cha
        if (isMiss(objClass)) {
            // kiểm tra xem đã chọn khóa hay chưa
            ...
            // thông báo về trang cha
            bootbox.alert("Chưa chọn lớp", function () { that.props.parent.ChooseTab('class') });
            return;
        }
        //Kiểm tra việc lấy dữ liệu và lấy lại dữ liệu; hoặc chưa load, hoặc cha đã bị thay đổi
        if (this.state.loaded === null || this.state.parentinfo !== objClass) {
            this.setState({ parentinfo: objClass });//Trường hợp có thay đổi thì nhận lại mã mới
            this.loadData();
        ...
        if (!!objClass)
            this.props.parent.setParentTitle(this.props.parentTitle + '(' + objClass[0].NAME + ')');
    },
...
*/
    loadData: function (obj, selectobject) {
        ...
        if (!!obj.EMAILSEARCH) {
            email = obj.EMAILSEARCH.value;
            emailtype = obj.EMAILSEARCH.type;
        }
        if (!!obj.ADDRESSSEARCH) {
            address = obj.ADDRESSSEARCH.value;
            addresstype = obj.ADDRESSSEARCH.type;
        }
        $.ajax({
            url: "/Student/getAllSearch",
            dataType: 'json',
            data: {
                page: page
                , pageSize: pageSize
                , classcode: this.props.parent.getKeyValue('class')[0].CODE
                , code: code
                , codetype: codetype
                , name: name
                , nametype: nametype
    ...
    onKeyClick: function () {
        if (isMiss(this.state.rowSelect)) {
        ...
        if (this.state.rowSelect.length !== 1) {
            ...
            this.state.priority.SetInput(obj);
            this.state.priority.Show();
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.ret >= 0) {
                        ...
                        if (data.ret === -2) {
                            bootbox.alert("Sinh viên đã được thêm vào lớp môn học, cần xóa các menu con trước!", false);
                        }
                        else {
 ...
 */
    onSearch: function () {
        ...
        var dataSearch = [
                           ...
                           , { title: 'Email', id: 'EMAILSEARCH', type: 'text' }
                           , { title: 'Địa chỉ', id: 'ADDRESSSEARCH', type: 'text' }
        ];
...
*/
   ...
   */
    onPrint: function () {
        ...
        var parameter = "code=" + this.state.currentcode + "&reporttype=Student";
    ...
    render: function () {
        return (
                ...
                <StudentPopup setData={this.setData} onPre={this.onPrevClick} onNext={this.onNextClick} ReloadData={this.ReloadList} ref={this.props.id+'_Popup'} id={this.props.id+'_Popup'} parent={this.props.parent} />
                <StudentPriorityAdd setData={this.setData} onSave={this.SaveClick}  ref="staffPriorityAdd" id="staffPriorityAdd" ref={this.props.id+'_Priority'} id={this.props.id+'_Priority'} parent={this.props.parent} />
