    ...
    $.fn.DropDownTree = function (options) {
        ...
        function RenderData(data, element) {
            for (var i = 0 ; i < data.length ; i++) {
                ...
                if (!element.is("li")) {
                    element.append('<li id="TreeElement' + data[i].id + '"' + dataAttrs + '>' + (options.multiSelect ? '<i class="fa fa-square-o select-box" aria-hidden="true"></i>' : '') + '<a href="' + ((typeof data[i].href != "undefined" && data[i].href != null) ? data[i].href : '#') + '">' + data[i].title + '</a></li>');
                    if (data[i].data != null && typeof data[i].data != "undefined") {
                        $("#TreeElement" + data[i].id).append("<ul style='display:none'></ul>");
                        $("#TreeElement" + data[i].id).find("a").first().prepend('<span class="arrow">' + options.closedArrow + '</span>');
                        RenderData(data[i].data, $("#TreeElement" + data[i].id).find("ul").first());
                    } else if (options.addChildren) {
                        $("#TreeElement" + data[i].id).find("a").first().prepend('<span class="arrow">' + options.closedArrow + '</span>');
                    }
                }
                else {
                    element.find("ul").append('<li id="TreeElement' + data[i].id + '"' + dataAttrs + '>' + (options.multiSelect ? '<i class="fa fa-square-o select-box" aria-hidden="true"></i>' : '') + '<a href="' + ((typeof data[i].href != "undefined" && data[i].href != null) ? data[i].href : '#') + '">' + data[i].title + '</a></li>');
                    if (data[i].data != null && typeof data[i].data != "undefined") {
                        $("#TreeElement" + data[i].id).append("<ul style='display:none'></ul>");
                        $("#TreeElement" + data[i].id).find("a").first().prepend('<span class="arrow">' + options.closedArrow + '</span>');
                        RenderData(data[i].data, $("#TreeElement" + data[i].id).find("ul").first());
        ...
        $(options.element).on("click", ".select-box", function (e) {
            ...
            if ($(this).hasClass("fa-square-o")) {
                ...
                if (options.selectChildren) {
                    //Lấy tất cả con và cái hiện tại
                    console.log('this', this,'parents', $(this).parent,'parent-li', $(this).parent("li"));
                    var arr = $(this).parent("li").first().find(".select-box").parent("li");
                    var i = 0;
                    for (i = 0; i < arr.length; i++)
                    {
                        changed.push(arr[i]);
                    }
                    $(this).parent("li").first().find(".select-box").removeClass("fa-square-o");
                    $(this).parent("li").first().find(".select-box").addClass("fa-check-square-o");
                }
                else
                {
                    //Chỉ tính cái hiện tại
                    changed.push($(this).parent("li").first());//phần tử được thay đổi
                }
            } else {
                //will unselect
                checked = false;
                ...
                if (options.selectChildren) {
                    ...
                    $(this).parent("li").first().find(".select-box").addClass("fa-square-o");
                    $(this).parent("li").first().find(".select-box").removeClass("fa-check-square-o");
            ...
            options.checkHandler(changed, checked);
        });
